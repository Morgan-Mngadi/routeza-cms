...
function applyExcerpt(data: Record<string, any>) {
  const existing = String(data.excerpt ?? "").trim();
  if (existing) return;
  const fromContent = truncate(firstParagraph(data.content), MAX_EXCERPT_LENGTH);
  if (fromContent) {
    data.excerpt = fromContent;
  }
}

export default {
  beforeCreate(event: any) {
    applyExcerpt(event.params.data ?? {});
  },
